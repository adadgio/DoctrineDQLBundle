...
class Where
{
    ...
    public static function andWhere($baseAlias, $builder, array $where = array())
    {
        $uniq = 0;
        ...
        foreach ($where as $f => $value) {
            ...
            if ($field === '($OR)') {
                ...
                if (null === $paramValue) {
                    ...
                    $builder
                        ->andWhere($condition)
                        ->setParameter($type['parameter'][0], $value[0]) // $paramValue
                        ->setParameter($type['parameter'][1], $value[1]); // $paramValue

                } else {
                    $builder
                        ->andWhere($condition)
                        ->setParameter($type['field'], $value); // $paramValue
                }
